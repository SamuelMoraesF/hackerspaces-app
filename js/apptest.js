...
function hackmuda(hacknum){
	...
	while(count < urls.length && !done) {
		...
		.fail(function( jqxhr, textStatus, error ) {
			if(count == 2)
			$( '.errorarea' ).html('<div id="alerta" class="alert alert-warning alert-dismissible" role="alert"><button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span></button><strong>' + document.webL10n.get("error") + '</strong> (CORS error, contact the lab) Load cache</div>');
			else
			$( '.errorarea' ).html('<div id="alerta" class="alert alert-warning alert-dismissible" role="alert"><button type="button" class="close" data-dismiss="alert"><span aria-hidden="true">&times;</span></button><strong>' + document.webL10n.get("error") + '</strong> You should check your connection</div>');
		});
	}
}
